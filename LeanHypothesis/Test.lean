...
def testLeanStrategies : IO Unit := do
  ...
  -- Demonstrate counterexample display
  IO.println "\n=== Demonstrating Counterexample Display ==="
  IO.println "Testing a property that will fail (x > 100 for naturals):"
  let failResult ← runPropertyTest "nat" "lambda x: x > 100" 20
  IO.println s!"Result: {failResult}"
